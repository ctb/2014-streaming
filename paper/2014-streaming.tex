...
% verify that compare-pos scripts etc are looking at same things
% are Ns responsible for error profile ratio??
% makefile => save details of streaming error trimming to file
...
\caption{Unique k-mer counts (M) for raw and normalized data sets using a k-mer size
of 20 and the specified coverage cutoff.  Digital normalization reduces the
total number of k-mers being tracked.}
...
\caption{Unique k-mer counts for raw and normalized data sets using a
  ...
  normalization reduces the total number of reads considered informative.}
...
Error rates before and after trimming was estimated by mapping mismatches.}
\label{tab:trimming}
\end{table}
